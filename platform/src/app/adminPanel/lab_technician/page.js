...
import { X, ChevronDown, Loader2, UserCog, Phone, MapPin, ShieldCheck, FlaskConical, CheckCircle, XCircle, AlertTriangle, Info } from 'lucide-react';
...
function ToastContainer() {
  ...
  const cfg = {
    ...
    confirm: { bg: '#fff',    border: '#088395', color: '#176B87', Icon: AlertTriangle, label: 'Confirm' },
  };
          ...
          <div key={t.id} style={{
            ...
            {t.type === 'confirm' && (
              <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
                <button onClick={() => { t.resolve(false); remove(t.id); }} style={{ padding: '7px 18px', borderRadius: '8px', border: '1.5px solid #d1d5db', background: 'white', color: '#374151', fontWeight: 600, fontSize: '13px', cursor: 'pointer' }}>
                  Cancel
                </button>
                <button onClick={() => { t.resolve(true); remove(t.id); }} style={{ padding: '7px 18px', borderRadius: '8px', border: 'none', background: 'linear-gradient(135deg,#088395,#0a9fb5)', color: 'white', fontWeight: 700, fontSize: '13px', cursor: 'pointer', boxShadow: '0 2px 8px rgba(8,131,149,0.25)' }}>
                  Confirm
                </button>
              </div>
            )}
...
export default function LabTechnicianManagement() {
  ...
  const fetchLab = async () => {
    ...
    try {
      ...
      const data = await res.json();
      setAllLabs(data.labs.map(l => ({ Lab: l.Lab_ID, Lab_id: l._id })));
  ...
  const resetForm = () => {
    setNewUser({ name: "", email: "", phoneNumber: "", labAccess: [] });
  };
  ...
  const handleAddUser = async () => {
    ...
    try {
      ...
      setNewUser({ name: "", email: "", password: "", labAccess: [] });
      await fetchUsers();
      await fetchLab();
    } catch (err) {
      console.error("Add Lab Technician Error:", err);
      toast.error("Something went wrong while adding Lab Technician.");
    }
  };
  ...
  const handleUpdateUser = async () => {
    ...
    try {
      ...
      toast.error("Something went wrong while editing Lab Technician.");
    }
  };
  ...
  const handleLabSelect = (selectedLab) => {
    setNewUser((prev) => {
      const alreadySelected = prev.labAccess.some((lab) => lab.Lab_id === selectedLab.Lab_id);
      return {
        ...prev,
        ...
        {users && users.length > 0 ? (
          users.map((user) => (
            <div key={user.id} style={{
              ...
              boxShadow: '0 2px 8px rgba(8,131,149,0.07)', borderLeft: `4px solid ${C.primary}`, transition: 'box-shadow 0.2s',
            }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', flexWrap: 'wrap' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexWrap: 'wrap' }}>
                  ...
                  <span style={{
                    ...
                    ...(user.status === 'active' ? { backgroundColor: C.mint, color: '#065f46' } : user.status === 'inactive' ? { backgroundColor: '#fee2e2', color: '#991b1b' } : { backgroundColor: '#fef3c7', color: '#92400e' }),
                  }}>
                  ...
                  <div style={{ display: 'flex', gap: '6px' }}>
                    ...
                    <button
                      ...
                      style={{ width: 34, height: 34, border: 'none', borderRadius: '8px', backgroundColor: C.ice, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'transform 0.2s, background 0.15s', transform: expandedCard === user.id ? 'rotate(180deg)' : 'rotate(0deg)', color: C.dark }}>
                      <ChevronDown size={16} />
                    </button>
                  </div>
                </div>
        ...
        >
          ...
          >
            ...
            <div style={{ marginBottom: '1rem' }}>
              ...
              <input type="text" style={inputStyle} value={newUser.name}
            ...
            <div style={{ marginBottom: '1rem' }}>
              ...
              <input type="email" style={inputStyle} value={newUser.email}
            ...
            <div style={{ marginBottom: '1rem' }}>
              ...
              <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                <input type="text" style={{ ...inputStyle, flex: 1 }} value={newUser.password || ""}
            ...
            <div style={{ marginBottom: '1rem' }} ref={dropdownRef}>
              ...
              <div style={{ position: 'relative' }}>
                <div
                  ...
                  style={{ minHeight: '44px', padding: '6px 12px', border: `2px solid ${isDropdownOpen ? C.primary : '#e2e8f0'}`, borderRadius: '10px', backgroundColor: 'white', cursor: 'pointer', display: 'flex', flexWrap: 'wrap', gap: '6px', alignItems: 'center', transition: 'border-color 0.2s' }}
                >
                ...
                {isDropdownOpen && (
                  <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, marginTop: '4px', backgroundColor: 'white', border: '1px solid #e2e8f0', borderRadius: '10px', boxShadow: '0 4px 16px rgba(8,131,149,0.12)', maxHeight: '200px', overflowY: 'auto', zIndex: 1000 }}>
                    <div style={{ padding: '10px 14px', cursor: 'pointer', fontSize: '14px', color: '#6b7280', borderBottom: '1px solid #f3f4f6' }}
                    ...
                    {allLabs.map((labObj) => {
                      ...
                      return (
                        <div key={labObj.Lab_id} onClick={() => handleLabSelect(labObj)}
