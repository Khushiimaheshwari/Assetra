...
// ─── Mock Data ────────────────────────────────────────────────────────────────
const LOGS = [
  { id: "LOG-001", assetId: "LAB16-PC3",  assetName: "Dell Workstation #3",     action: "Issue Reported",  user: "Priya Sharma",   role: "Lab Technician", timestamp: "2025-03-06 09:14 AM", description: "System failing to boot after power outage. BIOS shows memory error on POST screen.", status: "Pending" },
  { id: "LOG-002", assetId: "SCI12-PR1",  assetName: "HP LaserJet Pro",          action: "Issue Resolved",  user: "Ravi Kumar",     role: "IT Support",     timestamp: "2025-03-06 10:45 AM", description: "Replaced jammed roller assembly and cleared paper path. Test print successful.", status: "Resolved" },
  { id: "LOG-003", assetId: "LAB02-SV1",  assetName: "Dell PowerEdge Server",    action: "Asset Updated",   user: "Anita Singh",    role: "Admin",          timestamp: "2025-03-05 03:22 PM", description: "Updated RAM from 32GB to 64GB. Asset specifications and AMC value revised.", status: "Updated" },
  { id: "LOG-004", assetId: "PHY05-OS1",  assetName: "Tektronix Oscilloscope",   action: "Issue Approved",  user: "Dr. Mehta",      role: "HOD",            timestamp: "2025-03-05 11:08 AM", description: "Repair request approved. Vendor visit scheduled for March 10th.", status: "Approved" },
  { id: "LOG-005", assetId: "ADMIN-AC5",  assetName: "BlueStar 1.5T Split AC",   action: "Issue Reported",  user: "Suresh Patel",   role: "Lab Technician", timestamp: "2025-03-04 02:55 PM", description: "Cooling efficiency dropped significantly. Possible refrigerant leak detected.", status: "Pending" },
  { id: "LOG-006", assetId: "LAB08-SC2",  assetName: "Canon imageFORMULA",       action: "Asset Created",   user: "Anita Singh",    role: "Admin",          timestamp: "2025-03-04 09:00 AM", description: "New scanner added to inventory under Science Department. AMC registered.", status: "Created" },
  { id: "LOG-007", assetId: "LAB16-PC3",  assetName: "Dell Workstation #3",     action: "Issue Approved",  user: "Dr. Mehta",      role: "HOD",            timestamp: "2025-03-03 04:10 PM", description: "Memory replacement approved. Vendor notified. ETA 2 business days.", status: "Approved" },
  { id: "LOG-008", assetId: "EE03-MM1",   assetName: "Fluke Multimeter Pro",     action: "Asset Updated",   user: "Deepak Nair",    role: "IT Support",     timestamp: "2025-03-03 01:33 PM", description: "Calibration certificate updated. Next due date set to March 2026.", status: "Updated" },
  { id: "LOG-009", assetId: "SCI12-PR1",  assetName: "HP LaserJet Pro",          action: "Issue Reported",  user: "Meena Joshi",    role: "Lab Technician", timestamp: "2025-03-02 08:45 AM", description: "Printer offline error recurring. Network card suspected faulty.", status: "Pending" },
  { id: "LOG-010", assetId: "NET01-SW1",  assetName: "Cisco Catalyst Switch",    action: "Asset Created",   user: "Anita Singh",    role: "Admin",          timestamp: "2025-03-01 11:20 AM", description: "New 48-port network switch deployed in Server Room B. IP configured.", status: "Created" },
  { id: "LOG-011", assetId: "LAB02-SV1",  assetName: "Dell PowerEdge Server",    action: "Issue Resolved",  user: "Ravi Kumar",     role: "IT Support",     timestamp: "2025-02-28 05:15 PM", description: "RAID array rebuilt after disk failure. Data integrity verified. System stable.", status: "Resolved" },
  { id: "LOG-012", assetId: "ADMIN-AC5",  assetName: "BlueStar 1.5T Split AC",   action: "Issue Approved",  user: "Dr. Mehta",      role: "HOD",            timestamp: "2025-02-27 10:05 AM", description: "Gas refill and servicing approved. BlueStar AMC engineer visit confirmed.", status: "Approved" },
  { id: "LOG-013", assetId: "PHY05-OS1",  assetName: "Tektronix Oscilloscope",   action: "Issue Reported",  user: "Suresh Patel",   role: "Lab Technician", timestamp: "2025-02-26 03:40 PM", description: "Display showing random flickers during high-frequency signal measurement.", status: "Pending" },
  { id: "LOG-014", assetId: "EE03-MM1",   assetName: "Fluke Multimeter Pro",     action: "Asset Created",   user: "Anita Singh",    role: "Admin",          timestamp: "2025-02-25 09:30 AM", description: "Multimeter added to EE lab inventory. Accessories catalogued.", status: "Created" },
  { id: "LOG-015", assetId: "NET01-SW1",  assetName: "Cisco Catalyst Switch",    action: "Asset Updated",   user: "Deepak Nair",    role: "IT Support",     timestamp: "2025-02-24 02:10 PM", description: "Firmware updated to v15.2. VLAN configuration backed up.", status: "Updated" },
];

const ASSETS = [...new Set(LOGS.map(l => l.assetId))];
const ACTIONS = ["Issue Reported", "Issue Resolved", "Issue Approved", "Asset Created", "Asset Updated"];
const PAGE_SIZE = 8;
...
const BADGE = {
  ...
  "Asset Created":  { bg: "#F0FDF4", text: "#15803D", border: "#4ADE80", dot: "#16A34A" },
  "Asset Updated":  { bg: "#F4F4F5", text: "#3F3F46", border: "#D4D4D8", dot: "#71717A" },
};
...
function Badge({ action }) {
  const c = BADGE[action] || BADGE["Asset Updated"];
  return (
        ...
        <div className="flex items-start justify-between px-5 py-4 border-b border-slate-100"
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm font-bold" style={{ color: "#176B87", fontFamily: "'Playfair Display', serif" }}>{log.id}</p>
            </div>
          </div>
          ...
          {tab === "details" ? (
            ...
            <div className="space-y-0">
              ...
              <div className="relative pl-5">
                <div className="absolute left-2 top-0 bottom-0 w-px" style={{ background: "linear-gradient(to bottom,#088395,#D1F8EF)" }} />
                {timeline.map((t, i) => (
                  <div key={t.id} className={`relative mb-5 ${t.id === log.id ? "opacity-100" : "opacity-70"}`}>
                    ...
                    <div className={`ml-3 rounded-xl p-3 transition-all ${t.id === log.id ? "ring-2 ring-[#088395]/30 shadow-sm" : ""}`}
                      ...
                      <div className="flex items-center justify-between mb-1.5">
                        ...
                        <span className="text-xs text-slate-400">{t.timestamp.split(" ")[0]}</span>
                      </div>
...
export default function LogsPage() {
  ...
  // ── Responsive sidebar offset (matches AMC Dashboard pattern exactly) ──
  ...
  const inputCls = "w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-[#088395]/30 focus:border-[#088395] transition-all placeholder-slate-400";
  ...
  return (
    <>
      ...
      <div style={containerStyle}>
        ...
        {/* ── Action Summary Bar ────────────────────────────────────── */}
        <div className="fade-in" style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2,1fr)' : 'repeat(5,1fr)', gap: '1rem', marginBottom: '1.25rem', animationDelay: ".1s" }}>
          {ACTIONS.map(a => {
            const c = BADGE[a];
        ...
        <section className="fade-in" style={{ borderRadius: '16px', border: '1px solid rgba(8,131,149,0.12)', boxShadow: '0 4px 6px -1px rgba(8,131,149,0.08)', overflow: 'hidden', background: 'rgba(255,255,255,0.97)', marginBottom: '1.5rem', animationDelay: ".15s" }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: isMobile ? '14px 16px' : '16px 20px', borderBottom: '1px solid #D1F8EF' }}>
            ...
            <p style={{ fontSize: '11px', color: '#94a3b8' }}>Click any row for details</p>
          </div>
                  ...
                  <tr key={log.id}
                    ...
                    <td style={{ padding: isMobile ? '10px 12px' : '12px 20px', whiteSpace: 'nowrap' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        ...
                        <div>
                          <p style={{ fontSize: '12px', fontWeight: '600', color: '#334155', margin: 0 }}>{log.timestamp.split(" ").slice(0,2).join(" ")}</p>
                          <p style={{ fontSize: '11px', color: '#94a3b8', margin: 0 }}>{log.timestamp.split(" ").slice(2).join(" ")}</p>
                        </div>
                      </div>
                    </td>
                    {/* Asset ID */}
                    ...
                    <td style={{ padding: isMobile ? '10px 12px' : '12px 20px', whiteSpace: 'nowrap' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <div style={{ width: '30px', height: '30px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px', fontWeight: '700', color: 'white', background: `hsl(${log.user.charCodeAt(0) * 7 % 360},55%,45%)`, flexShrink: 0, boxShadow: '0 2px 4px rgba(0,0,0,0.1)' }}>
